...
#   I 겹침     : 메모장이 조각을 덮지 못하거나, 조각을 클릭하면 위로 떠오르면 실패
...
# ==== 1. 세 조각을 찾는다 + 게이트 W: 전부 최상위(바탕화면 위젯)여야 한다 ====
$pieces = @{}
foreach ($k in @('cal', 'mini', 'status')) {
  ...
  $pieces[$k] = $p.Key
  $r = Get-Rect $p.Key
  # 실제 부모는 GetAncestor(GA_PARENT=1)로 읽는다(GetParent 는 소유자 특례가 있다).
  $par = [W32P]::GetAncestor($p.Key, 1)
  $isTop = ($par -eq [IntPtr]::Zero) -or ($par -eq [W32P]::GetDesktopWindow())
  $parCls = ''
  if (-not $isTop) { $sb2 = New-Object System.Text.StringBuilder 256; [void][W32P]::GetClassNameW($par, $sb2, 256); $parCls = $sb2.ToString() }
  Write-Host "($k) rect=$($r.Left),$($r.Top),$($r.Right),$($r.Bottom) 위젯(최상위)=$isTop $parCls"
  if (-not $isTop) { $failures.Add("W: ($k) 조각이 다른 창('$parCls')의 자식입니다 — 바탕화면 위젯이 아닙니다.") }
}
...
if (-not $np) {
  ...
  $cr9 = Get-Rect $pieces['cal']
  $ncx = [int](($cr9.Left + $cr9.Right) / 2); $ncy = [int](($cr9.Top + $cr9.Bottom) / 2)
  [void][W32P]::SetWindowPos($np.MainWindowHandle, [IntPtr]::Zero, $ncx - 250, $ncy - 200, 500, 400, 0)
  Start-Sleep -Seconds 2
  Save-Screen "$OutDir/8-메모장이-달력을-덮음.png"
  $pt9 = New-Object W32P+POINT; $pt9.X = $ncx; $pt9.Y = $ncy
  $hit1 = [W32P]::GetAncestor([W32P]::WindowFromPoint($pt9), 2)
  if ($hit1 -ne $np.MainWindowHandle) {
    $failures.Add("I: 메모장을 달력 위에 놓았는데 달력이 메모장을 가립니다 — 위젯이 항상-아래가 아닙니다.")
  } else {
    Click ($cr9.Left + 40) ($cr9.Bottom - 60) 1
    Start-Sleep -Milliseconds 900
    $hit2 = [W32P]::GetAncestor([W32P]::WindowFromPoint($pt9), 2)
